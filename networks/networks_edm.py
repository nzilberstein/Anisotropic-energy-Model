...
#----------------------------------------------------------------------------
# Homogeneous-instance normalization.

class GlobalLocalNorm(torch.nn.Module):
    def __init__(self, num_channels, num_groups=1, eps=1e-5, C=1.0, group_size=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        global_var = ((x - global_mean) ** 2).mean(dim=[1, 2, 3], keepdim=True)  # (B, 1, 1, 1)
        # Compute channel-wise mean and variance
        local_mean = x.mean(dim=[2, 3], keepdim=True)  # (B, C, 1, 1)
        local_var = ((x - local_mean) ** 2).mean(dim=[2, 3], keepdim=True)  # (B, C, 1, 1)
        # Compute normalization factor
        scale = torch.sqrt((global_var + self.eps) / (self.C * local_var + self.eps))  # (B, C, 1, 1)
        del global_var, local_var
        # Normalize each channel
        out = (x - local_mean) * scale
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        if self.anisotropic_noise:
            self.affine_spatial = torch.nn.Conv2d(in_channels=emb_channels, out_channels=out_channels*(2 if adaptive_scale else 1), kernel_size=1)
            self.affine_freq = torch.nn.Conv2d(in_channels=emb_channels, out_channels=out_channels*(2 if adaptive_scale else 1), kernel_size=1)
        else:
    ...
    def forward(self, x, emb_spatial, emb_frequency, idxs_spatial, idxs_frequency):
        orig = x
        x = self.conv0(silu(self.norm0(x)))
        if len(idxs_spatial) !=0:
            params_spatial = self.affine_spatial(emb_spatial)
        if len(idxs_frequency) !=0:
            params_freq = self.affine_freq(emb_frequency)
        # The noise with covariance diagonal in the frequency domain has only one channel, so we average all of the them
        # at the output of the embedding network
        if len(idxs_frequency) !=0:
            params_freq = torch.nn.functional.adaptive_avg_pool2d(params_freq, 1)
        #  The noise with covariance diagonal in the pixel domain requires a downsampling
        if len(idxs_spatial) !=0:
            if params_spatial.shape[-2:] != x.shape[-2:]:
                params_spatial = torch.nn.functional.interpolate(
                    params_spatial,
        ...
        if self.adaptive_scale:
            if len(idxs_spatial) != 0:
                scale_s, shift_s = params_spatial.chunk(chunks=2, dim=1)
                x[idxs_spatial] = silu(torch.addcmul(shift_s, self.norm1(x[idxs_spatial]), scale_s + 1))
            
            if len(idxs_frequency) != 0:
                scale_f, shift_f = params_freq.chunk(chunks=2, dim=1)
                x[idxs_frequency] = silu(torch.addcmul(shift_f, self.norm1(x[idxs_frequency]), scale_f + 1))
        else:
            if len(idxs_frequency) !=0:
                x[idxs_frequency] = silu(self.norm1(x[idxs_frequency]) * (1 + params_freq))
            if len(idxs_spatial) !=0:
                x[idxs_spatial] = silu(self.norm1(x[idxs_spatial]) * (1 + params_spatial))
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        model_channels      = 128, #128,          # Base multiplier for the number of channels.
        channel_mult        = [1,2,2,2], #[1,2,2,2],    # Per-resolution multipliers for the number of channels.
        channel_mult_emb    = 4,            # Multiplier for the dimensionality of the embedding vector.
        num_blocks          = 4, #4 #5            # Number of residual blocks per resolution.
        attn_resolutions    = [16], #[16] #[32,16]         # List of resolutions with self-attention.
        ...
        adaptive_scale      = False,        # If True, use adaptive scaling of activations based on timestep embeddings (DDPM++ only).
    ):
        ...
        self.anisotropic_noise = True #TODO: this is hardcodede. Need to be changed!
        self.bin_spectrum = False #TODO: this is hardcoded. Need to be changed!
        self.homogeneous_norm = False
        self.adaptive_scale = adaptive_scale # The original is false

        print(self.adaptive_scale)
        block_kwargs = dict(
            emb_channels=self.emb_channels, num_heads=1, dropout=dropout, skip_scale=np.sqrt(0.5), eps=1e-6,
        ...
        # Mapping.
        if self.anisotropic_noise:
            self.map_noise_frequency = AnisotropicGammaEmbeddingNet(fourier_dim=noise_channels, t_min=1e-9, t_max=1e3,
                                                          base_channels=model_channels, num_res_blocks=4,
                                                          out_embedding_channels=self.emb_channels, embedding_type=embedding_type)
            # For 256
            # self.map_noise_frequency = AnisotropicGammaEmbeddingNet(fourier_dim=noise_channels, t_min=1e-9, t_max=1e3,
            #                                               base_channels=model_channels, num_res_blocks=8,
            #                                               out_embedding_channels=self.emb_channels, embedding_type=embedding_type)
            self.map_noise_spatial = AnisotropicGammaEmbeddingNet(fourier_dim=noise_channels, t_min=1e-9, t_max=1e3,
                                                          base_channels=model_channels, num_res_blocks=4,
                                                          out_embedding_channels=self.emb_channels, embedding_type=embedding_type)
        else:
        ...
        if self.bin_spectrum:
            self.map_layer0 = Linear(in_features=noise_channels * 15, out_features=self.emb_channels * 15, **init)
            self.map_layer1 = Linear(in_features=self.emb_channels * 15, out_features=self.emb_channels * 15, **init)
        else:
            self.map_layer0 = Linear(in_features=noise_channels, out_features=self.emb_channels, **init)
            self.map_layer1 = Linear(in_features=self.emb_channels, out_features=self.emb_channels, **init)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if decoder_type == 'skip' or level == 0:
                ...
                if self.homogeneous_norm is True:
                    self.dec[f'{res}x{res}_aux_norm'] = GlobalLocalNorm(num_channels=cout, eps=1e-6)
                else:
                    self.dec[f'{res}x{res}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res}x{res}_aux_conv'] = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)

    def forward(self, x, noise_labels, type_cov_list, covariance=None, class_labels=None, augment_labels=None):
        # Mapping.
        idxs_spatial = (type_cov_list == 0).nonzero(as_tuple=True)[0]
        idxs_frequency = (type_cov_list == 1).nonzero(as_tuple=True)[0]   

        if self.bin_spectrum:
            noise_labels = covariance.get_bin_spectrum(-5,10) * noise_labels[:,None]
            emb = self.map_noise(noise_labels.view(-1)).view(noise_labels.shape[0], -1)
        else:
            if len(idxs_spatial) !=0:
                emb_spatial = self.map_noise_spatial(noise_labels[idxs_spatial])
            else:
                emb_spatial = torch.zeros(0, self.emb_channels, device=x.device, dtype=x.dtype)
            if len(idxs_frequency) !=0:
                emb_frequency = self.map_noise_frequency(noise_labels[idxs_frequency])
            else:
                emb_frequency = torch.zeros(0, self.emb_channels, device=x.device, dtype=x.dtype)
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                x = block(x, emb_spatial, emb_frequency, idxs_spatial, idxs_frequency) if isinstance(block, UNetBlock) else block(x)
                skips.append(x)
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                x = block(x, emb_spatial, emb_frequency, idxs_spatial, idxs_frequency)
        return aux
    
#------------------------------------------------------ MLP -----------------

class AnisotropicConditionedMLP(torch.nn.Module):
    """
    MLP conditioned on anisotropic noise embeddings (SongUNet-style),
    concatenated directly with 2D input.
    """
    ...
    def __init__(
        ...
        # Match SongUNet dimensions
        self.emb_channels = model_channels * channel_mult_emb
        noise_channels = model_channels * channel_mult_noise

        # === Anisotropic noise embeddings ===
        self.map_noise_spatial = AnisotropicGammaEmbeddingNet(
            fourier_dim=noise_channels,
            t_min=1e-9,
            t_max=1e3,
            base_channels=model_channels,
            num_res_blocks=4,
            out_embedding_channels=self.emb_channels,
            embedding_type=embedding_type,
        )

        self.map_noise_frequency = AnisotropicGammaEmbeddingNet(
            fourier_dim=noise_channels,
            t_min=1e-9,
            t_max=1e3,
            base_channels=model_channels,
        ...
        # === MLP ===
        layers = []
        in_dim = 2 + self.emb_channels  # (x1, x2) + embedding
        ...
        layers.append(torch.nn.Linear(in_dim, 2))  # score or vector field
    ...
    def forward(self, x, sigma, type_cov_list):
        """
        Args:
            x: (B, 2)
            sigma: (B,)
            type_cov_list: (B,) with {0: spatial, 1: frequency}
        """

        B = x.shape[0]
        device = x.device

        # Split indices like SongUNet
        idxs_spatial = (type_cov_list == 0).nonzero(as_tuple=True)[0]
        idxs_frequency = (type_cov_list == 1).nonzero(as_tuple=True)[0]

        emb = torch.zeros(B, self.emb_channels, device=device, dtype=x.dtype)

        if len(idxs_spatial) > 0:
            emb[idxs_spatial] = self.map_noise_spatial(sigma[idxs_spatial])

        if len(idxs_frequency) > 0:
            emb[idxs_frequency] = self.map_noise_frequency(sigma[idxs_frequency])

        # Concatenate embedding with input
        mlp_input = torch.cat([x, emb], dim=-1)

        return self.network(mlp_input)
#----------------------------------------------------------------------------
...
class UNetBlock_embedding(torch.nn.Module):
    def __init__(self,
        in_channels, out_channels, emb_channels, up=False, down=False, attention=False,
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
        resample_filter=[1,1], resample_proj=False, adaptive_scale=True,
        init=dict(), init_zero=dict(init_weight=0), init_attn=None,
        anisotropic_noise=False, bin_spectrum=False,
    ):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        ...
        self.conv1 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3, **init_zero)
...
class AnisotropicGammaEmbeddingNet(torch.nn.Module):
    ...
    def forward(self, t_map):
        ...
        if self.embedding_type is None:
            ...
            t_map_flat = t_map.view(-1) #[B*H*W]
            sin_embedding = self.fourier_embedding(t_map_flat) # [B*H*W, 64]
