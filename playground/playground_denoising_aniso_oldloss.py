...
def compute_denoising_error(ctx, test_dataloader, noise_level_sampler, noisy_sampler, time_tracker, test_batch_size=512):
    ...
    for i, batch in enumerate(noisy_loader(test_dataloader, noise_level_sampler, noisy_sampler, time_tracker, batch_size=test_batch_size), start=1):
        ...
        output_isotropic_oldloss = ctxs['denoiser-isotropic-oldloss-oldTWeedie-tinbox'].model.forward(input, create_graph=True)
...
def compute_and_plot_single_denoising(ctxs, test_dataloader, noise_level_sampler, noisy_sampler, time_tracker, test_batch_size=512):
    for batch in noisy_loader(test_dataloader, noise_level_sampler, noisy_sampler, time_tracker):
    ...
    axs[0].imshow(np.clip(input.noisy[1].view(3, 32, 32).permute(1, 2, 0).detach().cpu().numpy(), 0, 1))
    axs[0].set_title('Noisy')
    axs[0].axis('off')
    axs[1].imshow(np.clip(output_isotropic_oldloss.denoised[1].view(3, 32, 32).permute(1, 2, 0).detach().cpu().numpy(), 0, 1))
    denoising_error = (output_isotropic_oldloss.denoised[1].view(-1) - batch.clean[1].view(-1)).norm().item()
    print(denoising_error)
...
if __name__ == "__main__":
    args = load_args("test_mult_anisotropic_oldloss_oldTweedie_tinbox", step = "best")

    ctxs = {
        "denoiser-anisotropic-oldloss-oldTWeedie-tinbox": load_exp("test_mult_anisotropic_oldloss_oldTweedie_tinbox", step = "last"),
    }

    default_ctx = ctxs["denoiser-anisotropic-oldloss-oldTWeedie-tinbox"]
    ...
    train_dataloader, test_dataloader, dataset_info = load_data(
        ...
        train_batch_size=128, test_batch_size=test_batch_size, num_workers=args["num_workers"], seed=2
    )
    ...
    for idx in range(num_variances ):
        ...
        denoising_error = compute_denoising_error(ctxs['denoiser-anisotropic-oldloss-oldTWeedie-tinbox'], test_dataloader, noise_level_sampler, noisy_sampler, time_tracker, test_batch_size=test_batch_size)
    ...
    }, 'plots/denoising_error_anisotropic_oldloss_newTweedie_tinbox.pt')