...
def spatial_corr_covariance_testing(spatial_size: int, box_size: int, var_box: float, device: torch.device, half_box_size: float, var_clean: float = 1e-3, inp_mask_type = "half", matrix = None,
                                    num_boxes = 1, noise_levels = None) -> Covariance:
    ...
    elif inp_mask_type == "autoregressive":
...
def spatial_corr_covariance(spatial_size: int, num_boxes: int, noise_levels: NoiseLevel, device: torch.device) -> Covariance:
    """ Returns a spatial correlation covariance with the given diagonal. """
    # Normalize diagonal to have unit variance per pixel.
    type_mask = 2 #torch.randint(0, 2, (1,)).item()
    if type_mask == 0:
        matrix = build_box_mask(spatial_size, box_size, var_box, device=device, var_clean=var_clean)
    elif type_mask == 1:
        matrix = build_half_mask(spatial_size, device=device, var_up=var_box, var_down=var_clean, half_box_size = half_box_size)
    elif type_mask == 2:
        matrix, box_size, centers = build_dynamic_noise_mask(
            spatial_size=spatial_size,
            num_boxes=num_boxes,
            noise_levels=noise_levels,
            device=device,
...
def sample_covariances_from_distribution(batch_size: int, spatial_size: int, noise_level: NoiseLevel, num_boxes: int, noise_level_sampler: NoiseLevelSampler, frequency_component: bool, spatial_component: bool) -> List[Covariance]:
    covariances = []
    ...
    if spatial_component == True:
        for i in range(batch_size):
            ...
            num_boxes_options = [1, 4, 16, 64, 256, 1024, 4096] # For Celeba
            num_boxes = np.random.choice(num_boxes_options)
            noise_levels = noise_level_sampler.sample_noise_levels(torch.Size([int(num_boxes)]), device="cuda")
            covariance = spatial_corr_covariance(spatial_size=spatial_size, num_boxes=num_boxes, noise_levels=noise_levels, device="cuda")
            covariances.append(covariance)
    return covariances 
...
class AdditiveNoisySampler(NoisySampler):
    def sample_noisy(self, clean: torch.Tensor, noise_level: NoiseLevel, noise_shape: torch.Size = (), num_boxes: int = 1, noise_level_sampler: NoiseLevelSampler = None) -> torch.Tensor:
        # Handle shapes.
        ...
        noise = self.sample_noise(shape=full_noise_shape, batch_size=clean.shape[0], noise_level=noise_level, device=clean.device, num_boxes=num_boxes, noise_level_sampler=noise_level_sampler)  # (B..., T..., N..., D...)
...
class MultipleColoredGaussianSampler(WhiteGaussianSampler):
    def __init__(self, frequency_component = False, spatial_component = False):
        ...
        self.sampler_covariance = lambda batch_size, shape, noise_level, num_boxes, noise_level_sampler = None: sample_covariances_from_distribution(batch_size, shape, noise_level, num_boxes, noise_level_sampler, self.frequency_component, self.spatial_component)  # List of covariance objects, length B...

    def get_noise_covariances(self):
        return self.noise_covariances
    
    # TODO: I think I can unify the logic of both cases.
    def sample_noise(self, shape: torch.Size, batch_size: int, device: torch.device, noise_level: NoiseLevel, num_boxes: int, noise_level_sampler: NoiseLevelSampler) -> torch.Tensor:
        if len(noise_level.stddev.shape) == 5:
            ...
            self.noise_covariances = self.sampler_covariance(batch_size, shape[-1], noise_level, num_boxes=num_boxes, noise_level_sampler=noise_level_sampler)  # List of covariance objects, length B...
            noises = []
...
def noisy_batch(clean: torch.Tensor, noise_level: NoiseLevelSampler | torch.Tensor | List[float] | float = 0, noisy_sampler: NoisySampler = WhiteGaussianSampler(), num_noises: torch.Size | int = ()) -> Batch:
    ...
    if hasattr(noise_level, "sample_noise_levels"):  # For some reason isinstance() does not always work? (maybe with autoreload in notebooks).
        ...
        num_boxes_options = [1, 4, 16, 64, 256, 1024, 4096] # For Celeba
        num_boxes = np.random.choice(num_boxes_options)
        noise_levels = noise_level.sample_noise_levels(torch.Size([int(batch_shape[0])]), device=clean.device)  # (B..., T...)
    ...
    noisy = noisy_sampler.sample_noisy(clean, noise_levels, num_noises, num_boxes=num_boxes, noise_level_sampler=noise_level)  # (B..., T..., N..., D...)
    covariances = noisy_sampler.get_noise_covariances()
    return Batch(noise_level=noise_levels, clean=clean, noisy=noisy, noise_covariance=covariances)
