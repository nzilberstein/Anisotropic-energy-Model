...
def custom_pdb(current_rank):
    if current_rank == 0: # e.g., 0 for the main rank
        print(f"Rank {current_rank} hitting breakpoint. Other ranks will wait.", flush=True)
        pdb.set_trace()
    else:
        # All other ranks must wait for the debugging rank to proceed
        # This prevents deadlocks during collective operations
        print(f"Rank {current_rank} waiting for rank {0} to finish debugging.", flush=True)
        dist.barrier() # Ensure all ranks synchronize after the breakpoint
        print(f"Rank {current_rank} resumed after breakpoint.", flush=True)
...
class TrainingContext:
    ...
    def __init__(self, *additional_args, step=None, key_remap=None, seed=0, dataloaders=True, writer=True, **additional_kwargs) -> None:
        ...
        train_dataloader, test_dataloader, dataset_info = load_data(
            dataset=args.dataset, world_size=world_size, rank=local_rank, spatial_size=args.spatial_size, grayscale=args.grayscale, horizontal_flip=args.horizontal_flip, data_subset=eval(args.data_subset),
        ...
        if args.noise_covariance is not None:
            ...
            # self.noise_covariance: Covariance = spatial_corr_covariance(dataset_info=self.dataset_info, box_size=5, device=self.device)   
        ...
        self.model.network = torch.nn.parallel.DistributedDataParallel(self.model.network, device_ids=[local_rank], output_device=local_rank)
        self.network: nn.Module = self.model.network.module # Access the underlying module
...
def parse_args(*additional_args, **additional_kwargs) -> argparse.Namespace:
    ...
    parser.add_argument("--network", choices=["DnCNN", "UNet", "UNet_measurement", "GradResNet"], help="network architecture")
    ...
    parser.add_argument("--test-every", type=int, default=10_00, help="test every N steps")
    ...
    if output.noise_score is not None:
        if train_test == "train":
            if len(t.shape) == 1:
                ...
                phi = get_inv_tensor_from_list_covariances(batch.noise_covariance).unsqueeze(1)
                phi_minus_z = 0.5 * (phi - z ** 2)
                e = apply_power_to_list_covariances(batch.noise_covariance, output.noise_score - phi_minus_z, p=1)
                noise_loss = torch.mean(e ** 2, dim=(-1, -2, -3))  # (B, [1 + L])
...
def main():
    ...
    ctx = TrainingContext()
    with ctx as _:
        if ctx.rank == 0:  # Print only from main process
