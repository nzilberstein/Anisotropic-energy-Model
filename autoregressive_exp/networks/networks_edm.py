...
class GlobalLocalNorm(torch.nn.Module):
    ...
    #     # Reshape to group
    #     xg = x.view(B, G, S, H, W)  # (B, G, S, H, W)
    #     # Compute global mean and variance
    #     global_mean = x.mean(dim=[1, 2, 3], keepdim=True)  # (B, 1, 1, 1)
    #     global_var = ((x - global_mean) ** 2).mean(dim=[1, 2, 3], keepdim=True).unsqueeze(1)  # (B, 1, 1, 1, 1)
    #     # Compute channel-wise mean and variance
    #     local_mean = xg.mean(dim=[2, 3, 4], keepdim=True)  # (B, G, 1, 1, 1)
    #     local_var = ((xg - local_mean) ** 2).mean(dim=[2, 3, 4], keepdim=True)  # (B, G, 1, 1, 1)
    #     # Compute normalization factor
    #     scale = torch.sqrt((global_var + self.eps) / (self.C * local_var + self.eps))  # (B, G, 1, 1, 1)
    #     xg = (xg - local_mean) * scale  # (B, G, S, H, W)
    #     x_norm = xg.view(B, C, H, W) * self.weight.to(x.dtype).view(1, C, 1, 1)  # (B, C, H, W)
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        if self.homogeneous_norm is True:
            self.norm0 = GlobalLocalNorm(num_channels=in_channels, eps=eps)
        else:
        ...
        if self.homogeneous_norm is True:
            self.norm1 = GlobalLocalNorm(num_channels=out_channels, eps=eps)
        else:
        ...
        if self.num_heads:
            if self.homogeneous_norm is True:
                self.norm2 = GlobalLocalNorm(num_channels=out_channels, eps=eps)
            else:
    ...
    def forward(self, x, emb):
        ...
        if self.anisotropic_noise is False:
            params = self.affine(emb).unsqueeze(2).unsqueeze(3).to(x.dtype)
        else:
            params = self.affine(emb)
            # -- For the pink noise -- #
            # params = torch.nn.functional.adaptive_avg_pool2d(params, 1)
            # -- #
            # -- For the inpainting noise -- #
            # Compute mean over channels (dim 1)
            # params = emb.mean(dim=1, keepdim=True)
        ...
        if self.adaptive_scale:
            ...
            # x = torch.real(torch.fft.ifft2(fft_x * (1 + params), norm="ortho"))
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        if self.anisotropic_noise:
            self.map_noise = AnisotropicGammaEmbeddingNet(fourier_dim=noise_channels, t_min=1e-9, t_max=1e3,
                                                          base_channels=model_channels, num_res_blocks=5,
    ...
    def forward(self, x, noise_labels, covariance=None, class_labels=None, augment_labels=None):
        # Mapping.
        if self.bin_spectrum:
