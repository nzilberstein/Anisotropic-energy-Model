...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, required=False, choices=['CIFAR10', 'Celeba', 'ImageNet64'])
    ...
    for seed in range(0, 10):
        ...
        train_dataloader, test_dataloader, dataset_info = load_data(
            ...
            train_batch_size=test_batch_size, test_batch_size=test_batch_size, num_workers=args["num_workers"], seed=20
        )
        ...
        gamma_cov = str(0.02) + "posterior_exp" + str(idx_img) #6e-2
        batch_idx_post = seed
        ...
        for batch_idx, images in enumerate(test_dataloader):
            ...
            for model_, ctx in ctxs.items():
                ...
                if domain == "freq":
                    x_deblur = deblurring_id.apply_power(clean_images, p = -0.5)
                    x_init = deblurring_id.apply_power(x_deblur + 0.01 * torch.randn_like(images[0][0:batch_size,:,:,:]).cuda(), p = 0.5)
                    # x_init = x_init
                else:
                    ...
                    # cov = spatial_corr_covariance_testing(spatial_size=img_size, box_size=box_size, var_box=0.9**2, device=device, var_clean = 0, half_box_size = box_size, inp_mask_type=inp_mask_type)
                    # noise_level = NoiseLevel(variance=30**2)
                    # input = ModelInput(noisy=x_init, noise_level=noise_level, covariance=[cov] * batch_size)
                    # model_output = ctx.model.forward(input, create_graph = False)
                    # grid = torchvision.utils.make_grid(model_output.denoised.cuda().cpu(), nrow=8, padding=2)
                    ...
                    # plt.tight_layout()
                    # plt.savefig(f'samples/MMSE_images{batch_idx}.pdf')
                ...
                if sampler == "PC":
                    x = PC_sampler(x_init, ctx.model, sigmas, n_steps_each=n_steps_each, final_only=True,
                                ...
                                snr = snr, domain = domain, kernel_size = kernel_size, kernel_std = kernel_std, device = device)
                elif sampler == "adaptive":           
                    x = adaptive_sampler(x_init, ctx.model, sigma_init = 4, n_updates = 1000 ,n_steps_each=3, step_cov_update = 5e-2, 
                        temp = 1, final_only=False, inp_mask_type=inp_mask_type, box_size = box_size, device = device)
                ...
                torch.save(ald_data, f"samples/{dataset_name}_{box_size}_{gamma_cov}_{sampler}_{domain}/samples_posterior_{batch_idx_post}.pt")
            ...
            # if batch_idx % 1 == 0 and batch_idx > 0: #20
            if batch_idx == 0:
            # if batch_idx == 0:
                break
